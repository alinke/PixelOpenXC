...
//these items were from the special ford hackathon firmware and are not available in the public VI firmware
...
public class MainActivity extends IOIOActivity implements TextToSpeech.OnInitListener  {
  ...
  //  final static int ACCEL11 = 11;
   ...
   // private BeepRequest beepCommand = new BeepRequest(BeepRequest.true);
    ...
    //public static final int MY_PERMISSIONS_REQUEST_RECEIVE_SMS = 456;
 ...
 /*   public void onRequestPermissionsResult(int requestCode,
        ...
        switch (requestCode) {
            case MY_PERMISSIONS_REQUEST_RECORD_AUDIO: {
                ...
                if (grantResults.length > 0
                    ...
                    alert.setTitle("Record Audio").setIcon(R.drawable.icon).setMessage("Hey there, this app will function but the voice commands for the LED display will not work.").setNeutralButton("OK", null).show();
            ...
            case MY_PERMISSIONS_REQUEST_RECEIVE_SMS: {
                ...
                if (grantResults.length > 0
                    ...
                    AlertDialog.Builder alert=new AlertDialog.Builder(this);
                    alert.setTitle("SMS Access").setIcon(R.drawable.icon).setMessage("This app will function but will not display text messages\n\n").setNeutralButton("OK", null).show();
                }
                return;
            }
   ...
   /* public static boolean hasPermissions(Context context, String... permissions) {
	...
	protected void onCreate(Bundle savedInstanceState) {
      ...
      /*  //let's request the three permissions we need here
...
final Runnable TongueRunnable = new Runnable() {
  ...
  //  _fbombEnable = prefs.getBoolean("pref_fbombEnable", false);
    ...
    case 2:
   	 ...
   	 break;
    case 3:
   	 KIND = ioio.lib.api.RgbLedMatrix.Matrix.ADAFRUIT_32x32; //v2
   	 BitmapInputStream = getResources().openRawResource(R.raw.openxcgrey);
   	 break;
    default:	    		 
   	 KIND = ioio.lib.api.RgbLedMatrix.Matrix.ADAFRUIT_32x32; //v2 as the default, it has 2 IDC connectors
   	 BitmapInputStream = getResources().openRawResource(R.raw.openxcgrey);
    }
	    ...
	    public void receive(Measurement measurement) {
	        ...
	        MainActivity.this.runOnUiThread(new Runnable() {
	            public void run() {
	            	...
	            	ignitionString = getValueStringSingle(ignitionString);
...
/*	GearLeverPosition.Listener mGearListener = new GearLeverPosition.Listener() {  //play rain animation
		...
		final VehicleDoorStatus _vDoorStatus = (VehicleDoorStatus) measurement;
		    ...
		    MainActivity.this.runOnUiThread(new Runnable() {
		            ...
		            public void run() {
		              ...
		              doorVal = getValueStringSingle(doorVal);
    ...
    public String getValueStringSingle(String str ) {  //format is like this "Percentage{value=1.23}" so our job here is to extract the 1.23 and convert that to an int
        String[] valuePair = str.split("=");
        String Value = valuePair[0]; //1.23}
        Value = Value.replace("}", ""); //take out the last bracket
        Value = Value.trim(); //trim
        // int pInt = Integer.parseInt(pedalValue); //now convert to int
        return(Value);
...
//	}
	...
	public void onInit(int status) {
       ...
       /* //during the ford hackathon, they gave us a special firmware where we could write to the car like setting the climate but this firmware is not available to the public
		mSoundPool.stop(mStream1);
